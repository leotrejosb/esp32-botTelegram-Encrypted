...
import json
import hashlib
from Crypto.Cipher import AES  # Necesita pycryptodome
import time
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(
            "Por favor, instala 'pycryptodome' (pip install pycryptodome) en el entorno Python donde ejecutas este script.")
        exit()
    except Exception as e:
    ...
    # Usar un client_id único para evitar desconexiones si otro cliente usa el mismo ID
    mqtt_client_id = f"mqtt_a_postgres_logger_{os.getpid()}"
    client = mqtt.Client(client_id=mqtt_client_id)
    # client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION1, client_id=mqtt_client_id) # Para paho-mqtt >= 2.0.0
    ...
    try:
        ...
        client.disconnect()
        print("Cliente MQTT desconectado. Saliendo.")